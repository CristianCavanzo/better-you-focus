import { BlockStatus, FocusState, TaskStatus } from '@/src/types/focus';
import { cuidLike, nowIso } from '@/src/lib/ids';
import { RepeatCadence } from '@prisma/client';
...
export function updateCategoryDefaultSeconds(
    ...
    const next = state.categories.map((c) =>
        c.id === categoryId
            ? { ...c, defaultSeconds: clamp(defaultSeconds, 60, 4 * 60 * 60) }
            : c,
    );
...
export function addTask(
    ...
    const task = {
        ...
        priority: clampInt(opts?.priority ?? 2, 1, 4) as 1 | 2 | 3 | 4,
...
export function updateTaskPriority(
    ...
    const p = clampInt(priority, 1, 4) as 1 | 2 | 3 | 4;
    const tasks = state.tasks.map((t) =>
        t.id === taskId ? { ...t, priority: p } : t,
    );
    return touch({ ...state, tasks });
}

export function updateTaskDueAt(
    state: FocusState,
    taskId: string,
    dueAt: string | null,
): FocusState {
...
export function updateTaskEstimate(
    ...
    const v =
        estimateMinutes == null ? null : clampInt(estimateMinutes, 1, 8 * 60);
    const tasks = state.tasks.map((t) =>
        t.id === taskId ? { ...t, estimateMinutes: v } : t,
    );
    return touch({ ...state, tasks });
}
...
export function getDraftBlock(state: FocusState, categoryId: string) {
    return (
        [...state.blocks]
            .reverse()
            .find((b) => b.status === 'DRAFT' && b.categoryId === categoryId) ??
        null
    );
}

export function ensureDraftBlock(
    state: FocusState,
    categoryId: string,
    ...
    if (draft) {
        ...
        return touch({
            ...
            blocks: state.blocks.map((b) =>
                b.id === draft.id
                    ? { ...b, plannedSeconds: plannedSeconds }
                    : b,
            ),
        });
    }
    ...
    return selections
        ...
        .sort(
            (a, b) =>
                ...
                a.selection.sortOrder - b.selection.sortOrder,
        );
}
...
export function isTaskInBlock(
    state: FocusState,
    blockId: string,
    taskId: string,
) {
    return state.selections.some(
        (s) => s.blockId === blockId && s.taskId === taskId,
    );
}

export function addTaskToBlock(
    state: FocusState,
    blockId: string,
    taskId: string,
): FocusState {
    ...
    const sortOrder = current.length
        ? Math.max(...current.map((s) => s.sortOrder)) + 1
        : 0;
...
export function removeTaskFromBlock(
    state: FocusState,
    blockId: string,
    taskId: string,
): FocusState {
...
export function toggleTaskInBlock(
    state: FocusState,
    blockId: string,
    taskId: string,
): FocusState {
...
export function startBlock(
    ...
    if (existingSelections.length === 0) {
        ...
        next = {
            ...next,
            tasks,
            selections: [...next.selections, ...newSelections],
        };
    }
